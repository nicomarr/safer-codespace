...
URL Source: https://llm.datasette.io/en/stable/usage.html
...
Executing a prompt[#](https://llm.datasette.io/en/stable/usage.html#executing-a-prompt "Link to this heading")
--------------------------------------------------------------------------------------------------------------

These examples use the default OpenAI `gpt-4o-mini` model, which requires you to first [set an OpenAI API key](https://llm.datasette.io/en/stable/setup.html#api-keys).

You can [install LLM plugins](https://llm.datasette.io/en/stable/plugins/installing-plugins.html#installing-plugins) to use models from other providers, including openly licensed models you can run directly on your own computer.
...
For models that support them, [system prompts](https://llm.datasette.io/en/stable/usage.html#usage-system-prompts) are a better tool for this kind of prompting.

### Model options[#](https://llm.datasette.io/en/stable/usage.html#model-options "Link to this heading")
...
You can also [configure default options](https://llm.datasette.io/en/stable/usage.html#usage-executing-default-options) for a model using the `llm models options` commands.

### Attachments[#](https://llm.datasette.io/en/stable/usage.html#attachments "Link to this heading")
...
### System prompts[#](https://llm.datasette.io/en/stable/usage.html#system-prompts "Link to this heading")
...
System prompts can be saved as [templates](https://llm.datasette.io/en/stable/templates.html#prompt-templates) to create reusable tools. For example, you can create a template called `pytest` like this:
...
See [prompt templates](https://llm.datasette.io/en/stable/templates.html#prompt-templates) for more.

### Tools[#](https://llm.datasette.io/en/stable/usage.html#tools "Link to this heading")

Many models support the ability to call [external tools](https://llm.datasette.io/en/stable/tools.html#tools). Tools can be provided [by plugins](https://llm.datasette.io/en/stable/plugins/plugin-hooks.html#plugin-hooks-register-tools) or you can pass a `--functions CODE` option to LLM to define one or more Python functions that the model can then call.
...
### Schemas[#](https://llm.datasette.io/en/stable/usage.html#schemas "Link to this heading")
...
Take a look at the [schemas documentation](https://llm.datasette.io/en/stable/schemas.html#schemas) for a detailed guide to using this feature.
...
Or use LLM’s custom [concise schema syntax](https://llm.datasette.io/en/stable/schemas.html#schemas-dsl) like this:
...
Or save your schema [to a template](https://llm.datasette.io/en/stable/templates.html#prompt-templates) like this:
...
See [Browsing logged JSON objects created using schemas](https://llm.datasette.io/en/stable/schemas.html#schemas-logs) for tips on using the `llm logs --schema X` command to access JSON objects you have previously logged using this option.

### Fragments[#](https://llm.datasette.io/en/stable/usage.html#fragments "Link to this heading")
...
Fragments are designed for running longer prompts. LLM [stores prompts in a database](https://llm.datasette.io/en/stable/logging.html#logging), and the same prompt repeated many times can end up stored as multiple copies, wasting disk space. A fragment will be stored just once and referenced by all of the prompts that use it.
...
### Continuing a conversation[#](https://llm.datasette.io/en/stable/usage.html#continuing-a-conversation "Link to this heading")
...
### Tips for using LLM with Bash or Zsh[#](https://llm.datasette.io/en/stable/usage.html#tips-for-using-llm-with-bash-or-zsh "Link to this heading")
...
### Completion prompts[#](https://llm.datasette.io/en/stable/usage.html#completion-prompts "Link to this heading")
...
Starting an interactive chat[#](https://llm.datasette.io/en/stable/usage.html#starting-an-interactive-chat "Link to this heading")
----------------------------------------------------------------------------------------------------------------------------------
...
Each chat starts a new conversation. A record of each conversation can be accessed through [the logs](https://llm.datasette.io/en/stable/logging.html#logging-conversation).
...
You can also pass [a template](https://llm.datasette.io/en/stable/templates.html#prompt-templates) - useful for creating chat personas that you wish to return to.
...
`llm chat` takes the same `--tool/-T` and `--functions` options as `llm prompt`. You can use this to start a chat with the specified [tools](https://llm.datasette.io/en/stable/usage.html#usage-tools) enabled.

Listing available models[#](https://llm.datasette.io/en/stable/usage.html#listing-available-models "Link to this heading")
--------------------------------------------------------------------------------------------------------------------------

The `llm models` command lists every model that can be used with LLM, along with their aliases. This includes models that have been installed using [plugins](https://llm.datasette.io/en/stable/plugins/index.html#plugins).
...
Setting default options for models[#](https://llm.datasette.io/en/stable/usage.html#setting-default-options-for-models "Link to this heading")
----------------------------------------------------------------------------------------------------------------------------------------------
...
Default model options are respected by both the `llm prompt` and the `llm chat` commands. They will not be applied when you use LLM as a [Python library](https://llm.datasette.io/en/stable/python-api.html#python-api).
