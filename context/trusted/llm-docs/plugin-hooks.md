...
Exposing tools to LLMs **carries risks**! Be sure to read the [warning below](https://llm.datasette.io/en/stable/tools.html#tools-warning).

How tools work[#](https://llm.datasette.io/en/stable/tools.html#how-tools-work "Link to this heading")
------------------------------------------------------------------------------------------------------
...
Trying out tools[#](https://llm.datasette.io/en/stable/tools.html#trying-out-tools "Link to this heading")
----------------------------------------------------------------------------------------------------------
...
Further tools can be installed using plugins, or you can use the `llm --functions` option to pass tools implemented as PYthon functions directly, as [described here](https://llm.datasette.io/en/stable/usage.html#usage-tools).

LLM’s implementation of tools[#](https://llm.datasette.io/en/stable/tools.html#llm-s-implementation-of-tools "Link to this heading")
------------------------------------------------------------------------------------------------------------------------------------
...
Tools can also be defined as [toolbox classes](https://llm.datasette.io/en/stable/python-api.html#python-api-toolbox), a subclass of `llm.Toolbox` that allows multiple related tools to be bundled together. Toolbox classes can be be configured when they are instantiated, and can also maintain state in between multiple tool calls.

The Python API can accept functions directly. The command-line interface has two ways for tools to be defined: via plugins that implement the [register_tools() plugin hook](https://llm.datasette.io/en/stable/plugins/plugin-hooks.html#plugin-hooks-register-tools), or directly on the command-line using the `--functions` argument to specify a block of Python code defining one or more functions - or a path to a Python file containing the same.

You can use tools [with the LLM command-line tool](https://llm.datasette.io/en/stable/usage.html#usage-tools) or [with the Python API](https://llm.datasette.io/en/stable/python-api.html#python-api-tools).

Default tools[#](https://llm.datasette.io/en/stable/tools.html#default-tools "Link to this heading")
----------------------------------------------------------------------------------------------------
...
Tips for implementing tools[#](https://llm.datasette.io/en/stable/tools.html#tips-for-implementing-tools "Link to this heading")
--------------------------------------------------------------------------------------------------------------------------------

Consult the [register_tools() plugin hook](https://llm.datasette.io/en/stable/plugins/plugin-hooks.html#plugin-hooks-register-tools) documentation for examples of how to implement tools in plugins.

If your plugin needs access to API secrets I recommend storing those using `llm keys set api-name` and then reading them using the [llm.get_key()](https://llm.datasette.io/en/stable/plugins/plugin-utilities.html#plugin-utilities-get-key) utility function. This avoids secrets being logged to the database as part of tool calls.
